...
application.use((request,response, next)=> {
    ...
    response.set('Access-Control-Allow-Methods','GET, POST, OPTIONS, PATCH, PUT, DELETE');
    next();
 })

 const token = async(request, response, next) => {
    try {
        next();
    } catch (error) {
        response.status(404).send('Please authentify');
    }
 }
...
// endpoint racine
application.get('/', token, (request, response) => response.send("Welcome to my first Node API"))

// endpoint of product
application.get('/product/', token,async (request, response) => {
    response.send(product);
})

// endpoint  product id
application.get('/product/:id', token,async (request, response) => {
    const id = request.params.id;
    if (id > product.length) {
        response.sendStatus(404);
    }else{
        const i = id - 1
        response.send(product[i]);
    }
})

// endpoint of product brand
application.get('/brand/',token,async (request, response) => {
    const brand = product.map((prod) => prod.brand);
    if (brand != undefined) {
        response.send(brand);
    }else{
        response.sendStatus(404);
